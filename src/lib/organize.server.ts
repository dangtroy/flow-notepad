      ...
      messages: [
        {
          ...
          content: [
            ...
            'Return "concepts": candidates for brand-new tags the user does not have yet. Emitting a candidate is silent evidence, not a suggestion — it only reaches the user after the same name recurs across notes. So emit reliably.',
            "ALWAYS emit a concept candidate when the note clearly centers on a specific, named proper noun: a person (Pierre, Sarah), a tool or platform (Shopify, Printify, ShipHero, Klaviyo), a brand, or a specifically named project. Emit even from a single note — candidates accumulate silently.",
            'For a person, use their bare name as the concept name and set kind = person. Always use the SAME name form ("Pierre", never "Pierre (professor contact)") so repeat mentions group together.',
            'Do NOT emit generic professional abstractions or category labels: "Web development", "E-commerce operations", "Quality Assurance", "Global Operation", "Product Data Management", "Email Marketing", "Marketing", "Operations". They are too vague to filter by.',
            "Rule of thumb: specific named thing (proper noun) → emit. Generic activity or category → skip. Consistency matters more than cleverness: the value is emitting the same normalized name across notes.",
            "Do not emit a concept that duplicates or is a close variant of an existing tag in the provided list, or of a previously ignored proposal. Emit at most 2 concepts per note.",
